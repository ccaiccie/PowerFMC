...
$srcNet=@()
if ($_.sourceNetworks.objects)  {$srcNet+= $_.sourceNetworks.objects.name}
if ($_.sourceNetworks.literals) {$srcNet+= $_.sourceNetworks.literals.value}
$srcNet = $srcNet -join ",`n"
$i | Add-Member -MemberType NoteProperty -Name SourceNetwork      -Value $srcNet
$dstNet=@()
if ($_.destinationNetworks.objects)  {$dstNet+= $_.destinationNetworks.objects.name}
if ($_.destinationNetworks.literals) {$dstNet+= $_.destinationNetworks.literals.value}
$dstNet = $dstNet -join ",`n"
$i | Add-Member -MemberType NoteProperty -Name DestinationNetwork -Value $dstNet
$srcPort=@()
if ($_.sourcePorts.objects)  {$srcPort+= $_.sourcePorts.objects.name}
if ($_.sourcePorts.literals) {$srcPort+= $MasterProtocolList[[int]$_.sourcePorts.literals.protocol] +'\'+$_.sourcePorts.literals.port}
$srcPort = $srcPort -join ",`n"
$i | Add-Member -MemberType NoteProperty -Name SourcePort -Value $srcPort
$dstPort=@()
if ($_.destinationPorts.objects)  {$dstPort+= $_.destinationPorts.objects.name}
if ($_.destinationPorts.literals) {$dstPort+= $MasterProtocolList[[int]$_.destinationPorts.literals.protocol] +'\'+$_.destinationPorts.literals.port}
$dstPort = $dstPort -join ",`n"
$i | Add-Member -MemberType NoteProperty -Name DestinationPort -Value $dstPort
